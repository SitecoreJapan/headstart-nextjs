...
const useOcCheckoutShipping = () =>
  useOcSelector((s) => {
    const { initialized, lineItems, order, shipEstimateResponse } =
      s.ocCurrentOrder;
  ...
  return (
    <div>
      {estimate.ShipMethods.map((method) => (
        <label
          key={method.ID}
          htmlFor={`estimate_${estimate.ID}_method_${method.ID}`}
        >
          <input
...
const OcShipEstimates: FunctionComponent = () => {
  ...
  const { orderId, shippingAddress, shipEstimateResponse } =
    useOcCheckoutShipping();
  ...
  const handleShipMethodSelectionChange = (
    selection: RequiredDeep<ShipMethodSelection>
  ) => {
