...
export const retrievePayments = createOcAsyncThunk<
  RequiredDeep<Payment>[],
  string
>("ocCurrentOrder/retrievePayments", async (orderId, ThunkAPI) => {
  const response = await Payments.List("Outgoing", orderId, { pageSize: 100 });
  return response.Items;
});

export const retrieveOrder = createOcAsyncThunk<
  RequiredDeep<OrderWorksheet> | undefined,
  void
>("ocCurrentOrder/retrieveOrder", async (_, ThunkAPI) => {
...
export const createLineItem = createOcAsyncThunk<
  ...
  LineItem
>("ocCurrentOrder/createLineItem", async (request, ThunkAPI) => {
  const { ocCurrentOrder } = ThunkAPI.getState();
...
export const removeLineItem = createOcAsyncThunk<
  RequiredDeep<OrderWorksheet>,
  string
>("ocCurrentOrder/removeLineItem", async (request, ThunkAPI) => {
  const { ocCurrentOrder } = ThunkAPI.getState();
  const orderId = ocCurrentOrder.order ? ocCurrentOrder.order.ID : undefined;
...
export const removeBillingAddress = createOcAsyncThunk<
  RequiredDeep<OrderWorksheet>,
  undefined
>("ocCurrentOrder/removeBillingAddress", async (request, ThunkAPI) => {
  const { ocCurrentOrder } = ThunkAPI.getState();
...
export const estimateShipping = createOcAsyncThunk<
  RequiredDeep<OrderWorksheet>,
  string
>("ocCurrentOrder/estimateShipping", async (orderId, ThunkAPI) => {
  const response = await IntegrationEvents.EstimateShipping(
    "Outgoing",
    orderId
  );
  return response;
...
export const submitOrder = createOcAsyncThunk<RecentOrder, any>(
  ...
  async (onSubmitted, ThunkAPI) => {
    const { ocCurrentOrder } = ThunkAPI.getState();
    const submitResponse = await Orders.Submit(
      "Outgoing",
      ocCurrentOrder.order.ID
    );
    // eslint-disable-next-line no-use-before-define
